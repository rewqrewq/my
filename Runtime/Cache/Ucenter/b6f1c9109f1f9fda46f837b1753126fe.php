...
<script src="/my/Public/js.php?f=js/jquery-2.0.3.min.js,js/com/com.functions.js,js/core.js,js/com/com.toast.class.js,js/com/com.ucard.js"></script>
    ...
    <link href="/my/Application/Ucenter/Static/css/usercenter.css" rel="stylesheet" type="text/css"/>
...
<!--顶部导航之后的钩子，调用公告等 end-->
    ...
    <?php if($user_info['cover_id']): ?><img class="uc_top_img_bg" src="<?php echo ($user_info['cover_path']); ?>" style="width: 100%;height: 100%">
        <?php else: ?>
        <img class="uc_top_img_bg" src="/my/Application/Ucenter/Static/images/user_top_default_bg.jpg" style="width: 100%;height: 100%"><?php endif; ?>
    <?php if(is_login() && $user_info['uid'] == is_login()): ?><div class="change_cover"><a data-type="ajax" data-url="<?php echo U('Ucenter/Public/changeCover');?>" data-toggle="modal" data-title="<?php echo L('_UPLOAD_PERSONAL_COVER_');?>" style="color: white;"><img class="img-responsive" src="/my/Application/Core/Static/images/fractional.png"></a>
        </div><?php endif; ?>
</div>
<div class="row uc_info">
    ...
    <?php if(is_login() && $user_info['uid'] != get_uid()): ?><div class="col-xs-3">
            <div class="uc_follow">
                <button type="button" class="btn btn-default" onclick="talker.start_talk(<?php echo ($user_info['uid']); ?>)"
    ...
    <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
        <ul class="nav navbar-nav">
            <?php if(empty($appArr)): ?><li>
                ...
                <li>
                    <a href="<?php echo U('following',array('uid'=>$uid));?>" class="<?php echo ($tabClass["user_fans"]); ?>"><?php echo L('_FOLLOWERS_'); echo L('_SLASH_'); echo L('_FANS_');?></a>
                </li><?php else: ?>
            <?php if(is_array($appArr)): $i = 0; $__LIST__ = $appArr;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li>
                    <?php switch($vo["key"]): case "follow": ?><a href="<?php echo U('following',array('uid'=>$uid));?>" class="<?php echo ($tabClass["user_fans"]); ?>"><?php echo ($vo["title"]); ?></a><?php break;?>
                        <?php case "info": ?><a href="<?php echo U('information',array('uid'=>$uid));?>" class="<?php echo ($tabClass["user_data"]); ?>"><?php echo ($vo["title"]); ?></a><?php break;?>
                        <?php case "rank_title": ?><a href="<?php echo U('rank',array('uid'=>$uid));?>" class="<?php echo ($tabClass["user_rank"]); ?>"><?php echo ($vo["title"]); ?></a><?php break;?>
                    <?php default: ?><a href="<?php echo U('appList',array('type'=>$key,'uid'=>$uid));?>"  class="<?php echo ($tabClass[$key]); ?>"><?php echo ($vo["title"]); ?></a><?php endswitch;?>
                </li><?php endforeach; endif; else: echo "" ;endif; endif; ?>
        </ul>
    </div>
    <!-- /.navbar-collapse -->
                    ...
                    <div class="row">
                        <div class="col-xs-12 uc_fans" style="margin-left: 10px;">
                            <ul class="nav nav-pills ucenter-tab">
                                <li <?php if(($tab) == "rank"): ?>class="active"<?php endif; ?>><a href="<?php echo U('Ucenter/Index/rank',array('uid'=>$uid));?>"

                                    >
                                <?php echo L('_TANK_OWNED_');?></a></li>
                                <?php if($uid == is_login()): ?><li <?php if(($tab) == "rankVerifyWait"): ?>class="active"<?php endif; ?>><a href="<?php echo U('Ucenter/Index/rankVerifyWait');?>"

                                        >
                                    <?php echo L('_RANK_WAIT_AUDIT_');?></a></li>
                                    <li <?php if(($tab) == "rankVerifyFailure"): ?>class="active"<?php endif; ?>><a href="<?php echo U('Ucenter/Index/rankVerifyFailure');?>"

                                        >
                                    <?php echo L('_RANK_APPLY_FAILED_');?></a></li>
                                    <li <?php if(($tab) == "rankVerify"): ?>class="active"<?php endif; ?>><a href="<?php echo U('Ucenter/Index/rankVerify');?>"

                                        >
                                    <?php echo L('_RANK_APPLY_');?></a></li><?php endif; ?>
                            </ul>
                        </div>
                    </div>
                    <?php if(count($rankList) == 0): ?><p class="text-muted"
                                                             style="text-align: center; font-size: 3em;">
                        <br><br>
                        <?php echo L('_RANK_NONE_'); echo L('_WAVE_');?>
                        <br><br><br>
                    </p><?php endif; ?>
                    <div style="margin: 20px 0 20px 20px;">
                        <?php if(is_array($rankList)): $i = 0; $__LIST__ = $rankList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$rank): $mod = ($i % 2 );++$i;?><div class="row"
                                ...
                                <div class="col-xs-2" style="margin-top: 30px;text-align: center;">
                                    <?php if(empty($rank["label_content"])): ?><img style="width: 18px;" class="rank-icon" src="<?php echo ($rank["logo_url"]); ?>" title="<?php echo ($rank["title"]); ?>" alt="<?php echo ($rank["title"]); ?>"/><?php else: ?><span class="label label-badge rank-label" title="<?php echo ($rank["title"]); ?>" style="background: <?php echo ($rank["label_bg"]); ?> !important;color:<?php echo ($rank["label_color"]); ?> !important;"><?php echo ($rank["label_content"]); ?></span><?php endif; ?>
                                </div>
                                <div class="col-xs-8">
                                    <div>
                                        <?php echo L('_RANK_NAME_'); echo L('_COLON_'); echo ($rank["title"]); ?>
                                    </div>
                                    <div style="margin-top: 10px;">
                                        <?php echo L('_REASON_'); echo L('_COLON_'); echo ($rank["reason"]); ?>
                                    </div>
                                    <div style="margin-top: 10px;">
                                        <?php echo L('_APPLY_TIME_'); echo L('_COLON_'); echo (friendlydate($rank["create_time"])); ?>
                                    </div>
                                </div>
                                <div class="col-xs-2">
                                    <?php if(($tab) == "rankVerifyWait"): ?><a class="btn btn-primary" href="<?php echo U('Ucenter/Index/rankVerifyCancel',array('rank_id'=>$rank['rank_id']));?>"><?php echo L('_APPLY_CANCEL_');?></a><?php endif; ?>
                                    <?php if(($tab) == "rankVerifyFailure"): ?><a class="btn btn-primary" href="<?php echo U('Ucenter/Index/rankVerify',array('rank_user_id'=>$rank['id']));?>"><?php echo L('_APPLY_RE_');?></a><?php endif; ?>
                                </div>
