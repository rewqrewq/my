...
<div class="panel-main" style="float:left;">
    ...
    <?php $default_url=U(ACTION_NAME,$_GET); ?>
<form id="searchForm" method="get" action="<?php echo ((isset($searchPostUrl) && ($searchPostUrl !== ""))?($searchPostUrl):$default_url); ?>" class="form-dont-clear-url-param">
...
<?php if(count($searches) > 0): ?><button class="btn submit-btn" url="?status=-1" target-form="ids" style="padding: 6px 16px;" onclick="toggle_search()"><?php echo L("_SEARCH_");?></button><?php endif; ?>
            ...
            <div style="float: right;" >
                ...
                <form id="selectForm" method="get" action="<?php echo ((isset($selectPostUrl) && ($selectPostUrl !== ""))?($selectPostUrl):$default_url); ?>" class="form-dont-clear-url-param">
    ...
    <div class="with-padding">
        <table id="table-data" class="table table-bordered table-striped table-hover">
            ...
            <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$e): $mod = ($i % 2 );++$i;?><tr>
                    ...
                    <?php if(is_array($keyList)): $i = 0; $__LIST__ = $keyList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$field): $mod = ($i % 2 );++$i;?><td style="width:auto;max-width: <?php echo ($field['width']); ?>;" class="text-ellipsis" <?php if(($field["opt"]) == "tooltip"): ?>data-title="<?php echo ($e[$field['name']]); ?>" data-role="watch"<?php endif; ?>><?php echo ($e[$field['name']]); ?></td><?php endforeach; endif; else: echo "" ;endif; ?>
