...
<!--顶部导航之后的钩子，调用公告等 end-->
    ...
    <?php echo W('Common/SubMenu/render',array($sub_menu,$current,array('icon'=>'th-large'),''));?>
    ...
    <?php if($display_type == 'masonry'): ?><div class="display_type">
            <a href="<?php echo U('Issue/Index/index',array('issue_id'=>$_GET['issue_id'],'display_type'=>'list'));?>" title=<?php echo L('_LIST_');?>><i class="icon-th"></i></a>
            <a class="current" title="<?php echo L('_FLOW_');?>"><i class="icon-align-justify"></i></a>
        </div>
        <?php else: ?>
        <div class="display_type">
            <a class="current" title="<?php echo L('_LIST_');?>"><i class="icon-th"></i></a>
            <a href="<?php echo U('Issue/Index/index',array('issue_id'=>$_GET['issue_id'],'display_type'=>'masonry'));?>" title=<?php echo L('_FLOW_');?>><i class="icon-align-justify"></i></a>
        </div><?php endif; ?>

    <div class="issue_list">
        <?php if(is_array($contents)): $i = 0; $__LIST__ = $contents;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="item_inner">
                <div class="item_core">
                    <div class="item_type"><?php echo ($vo["issue"]["title"]); ?></div>
                    <a href="<?php echo U('Issue/Index/issueContentDetail',array('id'=>$vo['id']));?>">
                        <?php if($display_type == 'masonry'): ?><img class="cover" src="<?php echo (getthumbimagebyid($vo["cover_id"],255)); ?>" height="<?php echo ((isset($vo["cover_height"]) && ($vo["cover_height"] !== ""))?($vo["cover_height"]):253); ?>"/>
                            <?php else: ?>
                            <img class="cover" src="<?php echo (getthumbimagebyid($vo["cover_id"],255,200)); ?>" style="height:200px;"/><?php endif; ?>
                    </a>

                    <div><h3><a href="<?php echo U('Issue/Index/issueContentDetail',array('id'=>$vo['id']));?>" class="text-more"
                                style="width: 100%"><?php echo ($vo["title"]); ?></a></h3></div>
                    <div class="spliter"></div>
                    <div>
                        <ul class="operation clearfix">
                            <li><i class="icon-eye-open"></i><?php echo ($vo["view_count"]); ?></li>
                            <li>
                                <?php echo Hook('support',array('table'=>'issue_content','row'=>$vo['id'],'app'=>'Issue','uid'=>$vo['uid'],'jump'=>'issue/index/issuecontentdetail'));?>
                            </li>
                            <li><i class="icon-comments-alt"></i><?php echo ($vo["reply_count"]); ?></li>
                        </ul>
                    </div>
                    <div class="spliter"></div>
                    <div><a class="author" href="<?php echo ($vo["user"]["space_url"]); ?>">
                        ...
                        <div class="pull-right ctime"><?php echo (friendlydate($vo["create_time"])); ?></div>
                    </div>
                </div>
            </div><?php endforeach; endif; else: echo "" ;endif; ?>
    </div>
        <?php if(count($contents) == 0): ?><div style="font-size:3em;padding:2em 0;color: #ccc;text-align: center"><?php echo L('_NOTHING_');?></div><?php endif; ?>
    <div>
...
<script src="/Public/js/ext/lazyload/lazyload.js"></script>
    ...
    <?php if($display_type == 'masonry'): ?><script src="/Application/Issue/Static/js/masonry/jquery.masonry.min.js" type="text/javascript"></script>
        <script type="text/javascript">
